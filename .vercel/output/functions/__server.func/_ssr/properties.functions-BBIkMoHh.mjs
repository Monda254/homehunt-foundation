...
import { i as isPlatformAdmin, t as hasPermission } from "./roles-BzUNBgvo.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/properties.functions-BBIkMoHh.js
/**
...
var getMyListings = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(getMyListings_createServerFn_handler, async ({ context }) => {
	...
	const { data: partyRows } = await supabaseAdmin.from("property_parties").select("property_id").eq("user_id", userId).eq("status", "ACTIVE");
	const propertyIds = (partyRows || []).map((r) => r.property_id);
	const orFilter = propertyIds.length > 0 ? `created_by_user_id.eq.${userId},property_id.in.(${propertyIds.join(",")})` : `created_by_user_id.eq.${userId}`;
	const { data } = await supabaseAdmin.from("listings").select("*, properties(name, county, town)").or(orFilter).is("deleted_at", null);
	return data || [];
});
